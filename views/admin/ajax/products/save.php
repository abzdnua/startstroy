    ...
    if($_POST['id']!=''){
      ...
      //  echo "error: ".$sql;
        ...
        $sql = "INSERT INTO products
                (name,des,price,priceForSale,category_id,material_id,firm_id,userCreate,dateCreate)
                VALUES
                ('{$name}','{$des}',{$price},{$priceForSale},{$category_id},{$material_id},{$firm_id},{$user},'{$date}')";
    //    echo "error: ".$sql;
        $db->query($sql);
        $id = $db->last();
