...
class db_manager
{
    ...
    public function getProductsArray($count = false,$category, $subCategory='', $start = '', $limit = ''){
        $db = $this->_db;
        if($start == '' AND $limit == '')
        {
            $l = '';
    ...
    public function getCategoryList($name, $parent = 0){
        $sql = "SELECT * FROM firms WHERE parent_id = ".$parent." AND deleted = 0";
        $this->_db->query($sql);
