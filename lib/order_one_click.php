...
if($_POST)
{
    $name = trim($_POST['name_one_click']);
    if($name == ''){
        echo json_encode(array('err'=>'Укажите ваше имя'));
        exit;
    }
    $phone = preg_replace('/[\s()]/','',$_POST['phone_one_click']);
    if($phone == '' OR strpos('_',$_POST['phone_one_click']))
    {
    ...
    $mess .= '<tr>';
    $mess .= '<td colspan="2" style="font-weight: bold">Заказ (в 1 клик)</td>';
    $mess .= '</tr>';
    $mess .= '<tr>';
    ...
    $mess .= '<tr>';
    $mess .= '<td style="width:200px">Итого<br />(с учетом всех скидок)</td>';
    ...
    $mess .= '<td>'.$_POST['phone_one_click'].'</td>';
    $mess .= '</tr>';
    $mess .= '</table>';
